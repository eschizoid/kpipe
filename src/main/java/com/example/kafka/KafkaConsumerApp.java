...
public class KafkaConsumerApp {
  private static final Logger LOGGER = Logger.getLogger(KafkaConsumerApp.class.getName());
  ...
  public static void main(String[] args) {
    final var bootstrapServers =
        getEnvOrDefault("KAFKA_BOOTSTRAP_SERVERS", DEFAULT_BOOTSTRAP_SERVERS);
    ...
    try {
      ...
      LOGGER.log(Level.SEVERE, "Fatal error in Kafka consumer application", e);
      System.exit(1);
    }
  }
  ...
  public KafkaConsumerApp(final String bootstrapServers, final String consumerGroup, String topic) {
    ...
    this.consumer =
        new FunctionalKafkaConsumer<>(
            kafkaProps, topic, wrapWithMetrics(processorPipeline), DEFAULT_POLL_TIMEOUT);
  }
        ...
        () -> {
          try {
            while (consumer.isRunning()) {
              LOGGER.info("Messages processed: " + MESSAGES_PROCESSED.get());
              ...
              registry
                  ...
                  .forEach(
                      processorName -> {
                        ...
                        if (!metrics.isEmpty()) {
                          LOGGER.info("Processor '" + processorName + "' metrics: " + metrics);
                        }
                      });
