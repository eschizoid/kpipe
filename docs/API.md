...
- Consumer-wide settings (`withRetry`, `withBackpressure`, `withProcessingMode`, `withKeyOrderedMaxKeys`, `withMetrics`,
  ...
  `withPollTimeout`) live on the builder. Setting one inside a route configurator throws `IllegalArgumentException`
  pointing you at the builder-level mirror — per-route values would silently fight over one consumer otherwise.
