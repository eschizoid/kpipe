...
The fluent surface, grouped by the type each method actually lives on. Chain shape: `KPipe.<format>(...)` → `Stream<T>`
(build the pipeline) → terminal → `Sink<T>` → `start()` → `Handle` (lifecycle). This catalog is verified against the
`kpipe-api` source; the [Javadoc](https://javadoc.io/doc/io.github.eschizoid/kpipe-api) is the normative reference.
...
Every method returns a **new immutable** `Stream<T>`, so two pipelines can branch from one prefix without affecting each
other.
...
| `withMetricsReporters(Collection<? extends KPipeMetricsReporter>)` | Periodic log-based reporting for deployments without an OTel backend; independent of `withMetrics`. The collection is copied, and the reporter thread is a daemon.                                                                                                                                       |
| `withMetricsInterval(Duration)`                                    | How often the reporters run. Only meaningful alongside `withMetricsReporters`; with no reporters the thread is never started.                                                                                                                                                                            |
| `withShutdownHook(boolean)`                                        | Registers a JVM shutdown hook to close the consumer. Try-with-resources on `Handle` already covers the normal exit; the hook covers SIGTERM.                                                                                                                                                             |
| `withThreadTerminationTimeout(Duration)`                           | How long shutdown waits for the consumer thread to join.                                                                                                                                                                                                                                                 |
| `withWaitForMessagesTimeout(Duration)`                             | How long shutdown waits for in-flight records to drain. Distinct from `Handle.shutdownGracefully(Duration)`, which bounds one specific drain call.                                                                                                                                                       |
...
- Consumer-wide settings (`withRetry`, `withBackpressure`, `withProcessingMode`, `withKeyOrderedMaxKeys`, `withMetrics`,
  `withTracer`, `withCircuitBreaker(CircuitBreakerController)`, `withDeadLetterTopic`, `withErrorHandler`,
  `withPollTimeout`, `withMetricsReporters`, `withMetricsInterval`, `withShutdownHook`, `withThreadTerminationTimeout`,
  `withWaitForMessagesTimeout`) live on the builder. Setting one inside a route configurator throws
...
`io.github.eschizoid.kpipe.registry.Operators` provides ready-made operators for `.pipe(...)`: `filter`, `drop`, `peek`,
`map`, `compose`, `safe`, `requireField`, `rename`, `removeFields`, `addField`. The `Map`-typed helpers (`rename`,
`removeFields`, `addField`, `requireField`) **mutate the payload map in place and return it** — they are convenient, not
pure functions. In the standard sink path this is sound (a record's payload is confined to the one worker processing
that attempt; the batch path hands buffered values to the flusher under a lock), but it matters if your own code shares
payload references beyond the pipeline.

## The explicit API

Everything above is a facade over `MessageProcessorRegistry` + `KPipeConsumerBuilder`. Drop down when you need custom
offset managers, pre-shared registries, custom Kafka consumer factories, periodic metrics reporters, or shutdown tuning
— the capability map and worked examples are in [ESCAPE-HATCHES.md](ESCAPE-HATCHES.md).
