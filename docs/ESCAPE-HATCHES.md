...
| Thread/executor termination tuning                  | `Stream.withThreadTerminationTimeout(Duration)` / `.withWaitForMessagesTimeout(Duration)`, and the `MultiBuilder` mirrors       | `Builder.withThreadTerminationTimeout / withWaitForMessagesTimeout`                                                        |
| Periodic metrics reporting + shutdown hook          | `Stream.withMetricsReporters(...)` / `.withMetricsInterval(...)` / `.withShutdownHook(boolean)`, and the `MultiBuilder` mirrors | `Builder.withMetricsReporters(...) / withMetricsInterval(...) / withShutdownHook(true)`                                    |
...
**Conditional processing** — `TypedPipelineBuilder.when(predicate, ifTrue, ifFalse)` branches inside one pipeline; both
branches are required. For drop-on-condition, use `filter` — a false predicate marks the record `Filtered` (offset still
