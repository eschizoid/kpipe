...
| **Periodic metrics reporting + shutdown hook**      | —                                                                        | `Builder.withMetricsReporters(...) / withMetricsInterval(...) / withShutdownHook(true)`                                    | Folded into `KPipeConsumer.Builder` in 1.13. Reporter thread is daemon, doesn't keep the JVM up. Behavior pinned by `KPipeReporterThreadTest` so the no-callers-in-repo signal can't be misread as dead code (#145). |
