...
/// Outcome of a [PartialBatchSink#apply] invocation. Reports per-record success and failure by
...
/// [BatchPipelineWrapper] enforces this — any indexes left unaccounted for are treated as
/// failures and routed to the configured DLQ. Implementations should not return overlapping
/// entries (an index appearing in both lists); behavior in that case is undefined.
///
...
public record BatchResult<T>(List<Integer> succeededIndexes, Map<Integer, Exception> failedByIndex) {
  public BatchResult {
  ...
  public static <T> BatchResult<T> allSucceeded(final int batchSize) {
    if (batchSize < 0) throw new IllegalArgumentException("batchSize cannot be negative, got " + batchSize);
    final var indexes = new java.util.ArrayList<Integer>(batchSize);
  ...
  public static <T> BatchResult<T> allFailed(final int batchSize, final Exception cause) {
    if (batchSize < 0) throw new IllegalArgumentException("batchSize cannot be negative, got " + batchSize);
    Objects.requireNonNull(cause, "cause cannot be null");
    final var failures = new java.util.HashMap<Integer, Exception>(batchSize);
