...
///
/// **Thread-safety.** A single [ReentrantLock] guards the buffer. `enqueue` may be called from many
/// virtual-thread workers concurrently (parallel and key-ordered modes) or serialized on the
// consumer
/// thread (sequential mode); the lock makes both safe. `tick` runs on the shared scheduler; `close`
